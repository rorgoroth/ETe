	...
	if ( flesh ) {
		...
		if ( headshot && cg_blood.integer ) {
			for ( i = 0; i < 5; i++ ) {
				...
				(void)CG_SmokePuff( origin, tmpv, 5 + rnd * 10, 1, rnd * 0.8, rnd * 0.8, 0.5, 500 + ( rand() % 800 ), cg.time, 0, 0, cgs.media.fleshSmokePuffShader );
			}
		} else {
			...
			for ( i = 0; i < 10; i++ ) {
				...
				tmpv2[2] = 0;
				VectorAdd( tmpv,tmpv2,tmpv );
				(void)CG_SmokePuff( origin, tmpv, 5 + rnd * 10,  rnd * 0.3f + 0.5f, rnd * 0.3f + 0.5f, rnd * 0.3f + 0.5f, 0.125f, 500 + ( rand() % 300 ), cg.time, 0, 0, cgs.media.smokePuffShader );
			}
		}
// jpw
